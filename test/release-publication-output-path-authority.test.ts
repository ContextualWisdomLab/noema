...
it("refuses to write a buyer release receipt through a symlinked output parent", () => {
  const temp = mkdtempSync(join(tmpdir(), "noema-release-output-parent-"));
  try {
    ...
    const realOutputParent = join(temp, "real-output-parent");
    const linkedOutputParent = join(temp, "linked-output-parent");
    const redirectedReceipt = join(realOutputParent, "release-publication-receipt.json");
    mkdirSync(realOutputParent);
    symlinkSync(realOutputParent, linkedOutputParent, "dir");
    const outputPath = join(linkedOutputParent, "release-publication-receipt.json");
    ...
    expect(result.stderr).toMatch(/output|parent|symlink/i);
    expect(existsSync(redirectedReceipt)).toBe(false);
  } finally {
