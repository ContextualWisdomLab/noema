...
const signingKid = "oidc-required-issued-at";
...
async function signedJwtWithoutIssuedAt(): Promise<string> {
...
describe("GitHub OIDC issued-at authority", () => {
  it("rejects a signed GitHub OIDC token that omits the provider-required iat claim", async () => {
    vi.resetModules();
    ...
    const response = await worker.fetch(
      new Request("https://noema.example/exchange", {
        method: "POST",
        headers: {
          authorization: `Bearer ${await signedJwtWithoutIssuedAt()}`,
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      message: "OIDC issued-at claim is invalid",
    });
  });
});
