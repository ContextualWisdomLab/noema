...
describe("GitHub Actions runner-assignment evidence source", () => {
  ...
  it("flattens all paginated jobs and rejects malformed page objects", () => {
    ...
    ).toEqual([{ id: 11 }, { id: 12 }, { id: 13 }]);

    expect(() => flattenJobPages([{ jobs: [] }, { unexpected: [] }])).toThrow(
      "jobs array",
    );
  });
    ...
    await expect(
      ...
      runs: [
        expect.objectContaining({
          id: 101,
          head_sha: expectedHead,
          jobs: [expect.objectContaining({ id: 1010, runner_id: 44 })],
        }),
        expect.objectContaining({
          id: 202,
          head_sha: expectedHead,
          jobs: [expect.objectContaining({ id: 2020, runner_id: 44 })],
        }),
      ],
    });
