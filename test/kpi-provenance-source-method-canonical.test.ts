...
describe("strict KPI provenance source method identity", () => {
  it("rejects an unreviewed sourceMethod instead of retaining it as production evidence", async () => {
    const directory = mkdtempSync(join(tmpdir(), "noema-kpi-source-method-"));
    ...
    try {
      ...
      expect(readFileSync(evidencePath, "utf8")).toContain(
        "KPI provenance sourceMethod must be one of the reviewed collection methods",
      );
    } finally {
