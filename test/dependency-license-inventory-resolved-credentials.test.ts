...
describe("dependency license inventory resolved artifact credentials", () => {
  ...
  it("fails closed when nested parameter inspection exceeds its bounded authority", () => {
    let nestedValue = "stable";
    for (let depth = 0; depth < 10; depth += 1) {
      nestedValue = `next=${encodeURIComponent(nestedValue)}`;
    }
    const resolved = `https://registry.example/alpha.tgz?mirror=${encodeURIComponent(nestedValue)}`;
