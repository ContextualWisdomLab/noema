...
describe("Context Graph released-contract admission", () => {
  ...
  it.each([
    ...
    unsafeEvidence({ releaseSourceManifestSha256: null }),
    releaseEvidence({ releaseSourceManifestSha256: "E".repeat(64) }),
    unsafeEvidence({ releaseSourceAttestationSha256: {} }),
    releaseEvidence({ releaseSourceAttestationSha256: "f".repeat(63) }),
    unsafeEvidence({ releaseSourceRef: [] }),
    releaseEvidence({ releaseSourceRef: "refs/heads/develop" }),
    unsafeEvidence({ releaseSourceSignerWorkflow: false }),
    releaseEvidence({
      releaseSourceSignerWorkflow:
        "ContextualWisdomLab/context-graph-contracts/.github/workflows/other.yml",
    }),
