...
const invalidUtf8Segment = Buffer.from([0xff]).toString("base64url");
...
describe("parseExactBearerToken canonical JWT segments", () => {
  ...
  it.each([
    ...
    ["invalid UTF-8 protected header", `${invalidUtf8Segment}.${canonicalPayload}.${canonicalSignature}`],
    ["invalid UTF-8 payload", `${canonicalHeader}.${invalidUtf8Segment}.${canonicalSignature}`],
