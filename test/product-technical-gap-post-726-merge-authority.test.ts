...
describe("post-726 protected authority convergence", () => {
  it("binds active authority to the new protected main and remaining open lanes", () => {
    ...
    expect(active).toContain("#730 current exact `09e795db5801b4d2cd8453f0e0d2f06b30189d59`");
    ...
    expect(active).toContain("requires the literal bullet reviewer credential line plus one blank line immediately before the canonical Noema marker and rejects bare credential lines");
    expect(active).toContain("requires formal Noema review authority to bind the exact evaluated base SHA as publisher-owned serialization and revalidate live state/head/base before publication");
    expect(active).toContain("requires the injectable Noema publisher seam to receive the same evaluated base SHA as the production publication path");
  ...
  it("binds governance candidate to the ordinary restack on the current protected main", () => {
    ...
    expect(row).toContain("candidate #730 exact `09e795db5801b4d2cd8453f0e0d2f06b30189d59`");
    ...
    expect(row).toContain("canonical bullet reviewer-credential serialization authority");
    expect(row).toContain("exact evaluated-base Noema review authority");
    expect(row).toContain("injected publisher evaluated-base parity authority");
