...
describe("release publication retained-input link authority", () => {
  it("rejects hard-linked publication inputs so an alternate pathname cannot mutate accepted evidence", () => {
    const temp = mkdtempSync(join(tmpdir(), "noema-publication-hardlink-"));
    try {
      const fixture = buildFixture(temp);
      linkSync(fixture.policyPath, join(temp, "policy-alias.json"));
      ...
      expect(result.stderr).toMatch(/single-link|hard link/i);
