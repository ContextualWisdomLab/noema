...
describe("product-technical gap baseline live open-PR authority", () => {
  it("treats #556 as integrated history and #560 as the active candidate", () => {
    ...
    expect(baseline).toContain("이 문서를 수렴시킬 때 관찰한 PR #560 exact는 `26634f181f9c29e875bdd57ab5bc046b109fd91b`");
    ...
    expect(baseline).toContain("core-conflict oracle은 `83ee8e9b54be7ea59cfa11fe03e4ad0c6414950c`");
    ...
    expect(baseline).toContain("동일 authority instance");
    expect(baseline).toContain("mutable test cache를 제거했다");
    ...
    expect(baseline).toContain("Canonical production `225a04ec2c833e9068e79ad7b70f3f5d8f6d934a`");
    expect(baseline).toContain("exact `AdmittedExternalExtension`-bound map");
    expect(baseline).toContain("Plaintext replay-retention RED `3f41d94af1454926b65b1801004fe245098742b0`");
    expect(baseline).toContain("Test-only `b50b43065098609118991e0b0b0b4936725c2899`");
    expect(baseline).toContain("Worker Web Crypto `crypto.subtle.digest(\"SHA-256\", ...)`");
    expect(baseline).toContain("application CI `34245484363`");
