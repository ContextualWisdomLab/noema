...
async function runGate(logPath: string, provenancePath: string, evidencePath: string) {
  ...
  try {
    ...
    for (const listener of process.listeners("exit")) {
      if (!priorExitListeners.has(listener)) {
        listener(0);
...
describe("authenticated KPI log line counting", () => {
  it("ignores whitespace-only separators and counts a final record without a trailing newline", async () => {
    const directory = mkdtempSync(join(tmpdir(), "noema-kpi-log-whitespace-"));
    try {
