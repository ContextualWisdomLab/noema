...
describe("observability alert input integrity", () => {
  ...
  it("rejects malformed JSON-looking lines instead of silently dropping evidence", () => {
    const dir = mkdtempSync(join(tmpdir(), "noema-observability-alert-"));
    try {
      const logPath = join(dir, "exchange.ndjson");
      writeFileSync(
        logPath,
        '{"event":"http_request","route":"/exchange","status_code":500,"latency_ms":120,"timestamp":"2026-08-01T00:00:00.000Z"\n',
      );

      const result = runAlerts(logPath);

      expect(result.status).toBe(1);
      expect(result.stderr).toContain("Malformed JSON in observability log line 1");
      expect(result.stdout).toBe("");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
