...
const signingKid = "oidc-issued-at-lifetime";
...
async function signedJwt(iat: number, exp: number): Promise<string> {
...
describe("GitHub OIDC issued-at lifetime", () => {
  it("rejects a still-unexpired signed token whose issued-at age exceeds the accepted one-hour lifetime", async () => {
    vi.resetModules();
    ...
    const response = await worker.fetch(
      new Request("https://noema.example/exchange", {
        method: "POST",
        headers: {
          authorization: `Bearer ${await signedJwt(now - 3_601, now + 300)}`,
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      message: "OIDC issued-at claim is outside the accepted lifetime window",
    });
  });
});
