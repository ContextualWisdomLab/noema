...
function activeAudit() {
  return {
    ...
    observed_at: "2026-08-16T03:10:00.000Z",
...
describe("workflow registry bounded GitHub reader", () => {
  it("rejects missing credentials and fetch capability before making a request", () => {
    ...
    expect(() => createWorkflowRegistryGithubJsonReader({ token: "delegated", fetchImpl: 0 as never }))
  ...
  it("pins reads to safe Noema repository endpoints", async () => {
    ...
    await expect(reader("repos\\ContextualWisdomLab/noema/actions/workflows"))
      .rejects.toThrow("endpoint is invalid");
    await expect(reader("https://example.com/repos/ContextualWisdomLab/noema/actions/workflows"))
      .rejects.toThrow("escapes the Noema repository boundary");
    await expect(reader("repos/ContextualWisdomLab/other/actions/workflows"))
      .rejects.toThrow("escapes the Noema repository boundary");
  ...
  it("returns validated JSON and sends the delegated token only in the request header", async () => {
    ...
    await expect(reader("repos/ContextualWisdomLab/noema/actions/workflows?per_page=100&page=1"))
      .resolves.toEqual({ total_count: 0, workflows: [] });
    expect(fetchImpl).toHaveBeenCalledTimes(1);
    const [url, options] = fetchImpl.mock.calls[0];
    expect(String(url)).toBe(
      "https://api.github.com/repos/ContextualWisdomLab/noema/actions/workflows?per_page=100&page=1",
    );
  ...
  it("maps timeout and network failures to bounded non-secret diagnostics", async () => {
    ...
    const timeoutReader = createWorkflowRegistryGithubJsonReader({
      token: "delegated-token",
      fetchImpl: vi.fn().mockRejectedValue(timeout),
    });
    await expect(timeoutReader("repos/ContextualWisdomLab/noema/actions/workflows"))
      .rejects.toThrow("request timed out");

    const networkReader = createWorkflowRegistryGithubJsonReader({
      token: "delegated-token",
      fetchImpl: vi.fn().mockRejectedValue(new Error("ghp_secret")),
    });
    await expect(networkReader("repos/ContextualWisdomLab/noema/actions/workflows"))
  ...
  it("fails closed on non-success, oversized, malformed UTF-8, duplicate-key, and invalid JSON bodies", async () => {
    ...
    const cases: Array<[ReturnType<typeof fakeResponse>, string]> = [
      ...
      [fakeResponse({ body: "{" }), "is not valid JSON"],
    ];
...
describe("immediate full workflow registry refresh", () => {
  it("rejects any repository or reader outside the exact Noema authority", async () => {
    ...
    await expect(collectLiveWorkflowRecords({ repository: REPOSITORY, ghJson: 0 as never }))
      .rejects.toThrow("restricted to ContextualWisdomLab/noema");
  });
  ...
  it("collects every paginated registry record and preserves page order", async () => {
    ...
    const ghJson = vi
      .fn()
      .mockResolvedValueOnce({ total_count: 101, workflows: firstPage })
      .mockResolvedValueOnce({ total_count: 101, workflows: [finalWorkflow] });
  ...
  it("fails closed when total count changes or a terminal page does not retain that count", async () => {
    const changedTotal = vi
      .fn()
      .mockResolvedValueOnce({ total_count: 101, workflows: [] })
      .mockResolvedValueOnce({ total_count: 100, workflows: [] });
    await expect(collectLiveWorkflowRecords({ repository: REPOSITORY, ghJson: changedTotal }))
      .rejects.toThrow("total changed during immediate pre-mutation refresh");

    await expect(collectLiveWorkflowRecords({
      repository: REPOSITORY,
      ghJson: vi.fn().mockResolvedValue({ total_count: 2, workflows: [{ id: 1 }] }),
    })).rejects.toThrow("did not retain the advertised record count");
  });
  ...
  it("bounds pathological pagination even when the remote count never terminates", async () => {
    ...
    await expect(collectLiveWorkflowRecords({ repository: REPOSITORY, ghJson }))
      .rejects.toThrow("pagination exceeded the bounded page limit");
...
describe("disablement input and postcondition boundaries", () => {
  it("rejects invalid repository, workflow identity, collectors, and transport", async () => {
    await expect(runWorkflowRegistryDisablement({ repository: "other", workflowId: 101 }))
      .rejects.toThrow("restricted to ContextualWisdomLab/noema");
    await expect(runWorkflowRegistryDisablement({ repository: REPOSITORY, workflowId: 0 }))
      .rejects.toThrow("positive safe integer");
    await expect(runWorkflowRegistryDisablement({ repository: REPOSITORY, workflowId: 101 }))
  ...
  it("does not mutate when fresh planning is non-authorizing", async () => {
    ...
    await expect(runWorkflowRegistryDisablement({
      ...
      collectLiveWorkflows: vi.fn().mockResolvedValue([
        { id: 101, path: ORPHAN_PATH, state: "active" },
        { id: 101, path: ".github/workflows/reused.yml", state: "active" },
      ]),
      transport,
