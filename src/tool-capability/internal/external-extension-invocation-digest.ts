...
function sha256Hex(value: string): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + sum1 + choose + (SHA256_ROUND[index] ?? 0) + (words[index] ?? 0)) >>> 0;
 ...
 *
 * The fixed field order is the canonicalization contract. The domain and version
 * prevent this digest from being confused with unrelated SHA-256 identities. Only
 * the fixed-width digest is retained with the receipt; plaintext request material
 * remains scoped to the invocation call and becomes collectible after it returns.
 */
export function digestExternalExtensionInvocationEnvelope(
  ...
  const digest = sha256Hex(`${DOMAIN}\0${VERSION}\0${canonical}`);
  return `${DOMAIN}:${VERSION}:sha256:${digest}`;
}
