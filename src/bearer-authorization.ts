 ...
 * JWT protected headers or payloads beginning with a UTF-8 BOM are rejected before any
 * claim reader can silently strip those signed bytes while interpreting JSON authority.
 *
...
export function parseExactBearerToken(authorization: string): string | undefined {
  ...
  if (
    segments.length === 3
    && (segments[0].startsWith(utf8BomBase64UrlPrefix) || segments[1].startsWith(utf8BomBase64UrlPrefix))
  ) return undefined;
  return token;
}
