 ...
 * effect-specific policy. A completion response is accepted only when it proves the same attempt
 * reached the observed terminal state after effect-start authority; stale or mismatched completion
 * evidence fails closed. This service does not retry, select providers, infer security/business truth,
 * or execute compensation on its own.
 *
