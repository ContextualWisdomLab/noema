...
`wrangler.toml` pins `ALLOWED_WORKFLOW_SHA` to protected central `.github` commit `a6fc45e98f8fdb7ccc90bc1d45addd9c8c9331a4`. The exact trusted `.github/workflows/noema-review.yml` blob at that protected commit is Git blob `ce46d82410cb6b01c2c08527d2445e4517c3d14a`. The preceding executable central change at `e4797ec08444fb5e005e88370e8f70509e955683` intentionally made Noema review independent of OpenCode/check/thread completion, fail closed on unverifiable or colliding reviewer identity and malformed/non-substantive LLM verdict evidence, bind GitHub App actor/installation identity into the review step, and require live-head evidence in runtime review acceptance. Movement from `e4797...` to `58d0c91b1bae3948e0155c3d19368b36bbf2c756` changed only `docs/doctoring/noema-orchestrator-free-zdr.md`; movement from `58d0c91...` to current `a6fc45...` changed central review-orchestration launcher policy/tests plus CHANGELOG while leaving the trusted `noema-review.yml` blob byte-identical. Noema re-audits both the exact workflow blob and relevant protected central delta before accepting each repository-commit movement because GitHub OIDC `job_workflow_sha` binds the caller to that exact repository commit identity. This is an explicit protected-ref plus exact-source trust decision, not a claim that unrelated central tree changes or future revisions are equivalent. The central repository remains a read-only dependency from Noema; known central consumer/scanner defects remain owned and repaired there rather than by weakening Noema verification.
