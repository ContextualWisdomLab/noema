...
`wrangler.toml` pins `ALLOWED_WORKFLOW_SHA` to protected central `.github` commit `e4797ec08444fb5e005e88370e8f70509e955683`. Before this candidate roll-forward, that exact protected commit was re-fetched and the trusted `.github/workflows/noema-review.yml` blob was verified as Git blob `ce46d82410cb6b01c2c08527d2445e4517c3d14a`. Relative to the previously trusted `4b115bdad2b3682f36639c4f978fd0e976a83ff0`, central review behavior intentionally changed: Noema review became independent of OpenCode/check/thread completion, the reviewer now fails closed on unverifiable or colliding reviewer identity and malformed/non-substantive LLM verdict evidence, GitHub App actor/installation identity is explicitly bound into the review step, and runtime review acceptance requires live-head evidence in the review receipt. This is an explicit protected-ref plus exact-workflow-content trust decision, not a claim that unrelated central tree changes or future revisions are equivalent. GitHub OIDC `job_workflow_sha` binds the caller to the repository commit identity, so every later central commit movement still requires a new explicit Noema trust roll-forward and fresh exact-head evidence. The central repository remains a read-only dependency from Noema; known central consumer/scanner defects remain owned and repaired there rather than by weakening Noema verification.
