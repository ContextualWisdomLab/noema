...
`wrangler.toml` pins `ALLOWED_WORKFLOW_SHA` to protected central `.github` commit `4a5dfd823c678f97f85f5e40a4bd12b202885d35`. The exact trusted `.github/workflows/noema-review.yml` blob at that protected commit remains Git blob `ce46d82410cb6b01c2c08527d2445e4517c3d14a`. Movement from `a6fc45e98f8fdb7ccc90bc1d45addd9c8c9331a4` to current `4a5dfd...` changed the central OpenCode required-review workflow, the review architecture ADR, `scripts/ci/noema_review_gate.py`, and focused review-gate regressions while leaving `noema-review.yml` byte-identical. The changed trusted source materially strengthens current-head review execution and requires formal Noema verdicts to carry exact changed-line analysis plus bounded adversarial probe evidence. Because `noema-review.yml` materializes the complete trusted central source tree at its immutable workflow SHA, this repository-commit movement is material even when the workflow YAML blob itself is unchanged. Noema therefore re-audits both the exact workflow blob and the relevant protected central delta before accepting each repository-commit movement; ancestry alone is insufficient because GitHub OIDC `job_workflow_sha` binds the caller to that exact repository commit identity. The central repository remains a read-only dependency from Noema. The protected central OIDC consumer still reads a top-level `.token` from the exchange response while Noema's stable success envelope exposes the credential under `data.token`; that consumer defect remains central-owned and is not repaired by weakening or reshaping Noema's stable envelope.
