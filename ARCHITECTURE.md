...
`wrangler.toml` pins `ALLOWED_WORKFLOW_SHA` to protected central `.github` commit `c8cc68a34bd19a91e2544acf08f2ead142ba702b`. The exact trusted `.github/workflows/noema-review.yml` blob at that protected commit is Git blob `064c4e5aeedcbb188196bd4800a0b918abd6da27`. Movement from `4a5dfd823c678f97f85f5e40a4bd12b202885d35` to current `c8cc68a...` changed the trusted workflow itself plus its focused contract test: the Noema review gate is now executed as the Python module `python3 -m scripts.ci.noema_review_gate` from the materialized immutable central source tree. This is a real trusted-workflow byte change, not an ancestry-only source movement, so Noema re-audited the new workflow blob and protected one-commit delta before accepting the new repository commit identity. More generally, because `noema-review.yml` materializes the complete trusted central source tree at its immutable workflow SHA, Noema re-audits both the exact workflow blob and every relevant protected central delta before each repository-commit movement; ancestry alone is insufficient because GitHub OIDC `job_workflow_sha` binds the caller to that exact repository commit identity. The central repository remains a read-only dependency from Noema. The protected central OIDC consumer still reads a top-level `.token` from the exchange response while Noema's stable success envelope exposes the credential under `data.token`; that consumer defect remains central-owned and is not repaired by weakening or reshaping Noema's stable envelope.
