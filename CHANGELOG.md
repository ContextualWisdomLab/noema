...
## Unreleased
- 비신뢰 repository의 CodeGraph 파싱을 digest-pinned Node 24 Docker 격리 경계로 이동. source/tooling/entrypoint는 read-only bind mount, root filesystem은 read-only, outbound network와 Linux capabilities는 차단하고 non-root UID·no-new-privileges·CPU/메모리/PID/ulimit·tmpfs·파일/바이트/출력/시간 quota를 강제하며 Docker subprocess에는 GitHub·Noema credential을 전달하지 않음.
