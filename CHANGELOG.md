...
## Unreleased
- `.github/workflows/hourly-commercial-readiness.yml`과 SHA-bound 유지보수 스크립트를 추가해 매시간 열린 PR의 리뷰 thread, current-head checks/status, Noema App verdict를 실패-폐쇄 방식으로 확인하고, 동일 head review dispatch를 중복하지 않으며 검증 완료 PR만 squash merge하도록 자동화.
