...
def replace_exact(path_text: str, old: str, new: str, count: int) -> None:
    path = Path(path_text)
    text = path.read_text(encoding="utf-8")
    observed = text.count(old)
    if observed != count:
        raise SystemExit(f"{path_text}: expected {count} matches, found {observed}")
...
old_guard = """          if git diff --cached --summary | grep -Eq 'mode (120000|160000)'; then
            echo "::error::A proposed symlink or gitlink (mode 120000|160000) is outside the bounded agent contract."
            exit 1
          fi
"""
new_guard = """          if git diff --cached --raw | awk '
            /^:/ {
              old_mode = substr($1, 2)
              new_mode = $2
...
"""
replace_exact(workflow, old_guard, new_guard, 3)
...
공식 OpenCode 아카이브는 고정 버전과 SHA-256으로 검증합니다. 공급자는 NVIDIA NIM 엔드포인트만 허용하고 공유, 자동 업데이트, MCP, LSP, 외부 디렉터리, 하위 에이전트, 질문 도구, 웹 검색과 웹 가져오기를 비활성화합니다. 모델 후보는 Llama Nemotron, Nemotron Super, DeepSeek 순서로 시도합니다. **후보별 900초**와 강제 종료 유예 30초를 적용합니다. 세 후보 2,700초, 종료 유예 90초, 설정·정리·재설치·최종 진단 예비 시간 300초를 합쳐도 55분 제안 job 안에 들어갑니다.

후보 실패 시 `git reset --hard HEAD`, `git clean -fdx`, `npm ci --ignore-scripts`를 실행한 뒤 다음 후보로 폴백합니다. 이전 후보의 부분 변경이나 생성 파일이 다음 후보의 입력을 오염시키지 않도록 합니다. 폴백은 가용성 제어일 뿐 품질 증거가 아니며, 성공한 제안도 독립 검증과 리뷰를 통과해야 합니다.
...
replace_exact(
    ...
    "Each model candidate has a 900-second timeout plus a 30-second forced-termination grace period. Three complete candidate budgets, cleanup, dependency reinstall, and the final all-candidates-failed diagnostic fit inside the 55-minute proposal-job limit. Failure triggers hard reset, ignored/untracked cleanup, and dependency reinstall before the next candidate. Partial output from one model cannot contaminate fallback input. Fallback improves availability; it is not quality evidence.\n",
    1,
)
Path(".github/workflows/apply-coderabbit-hourly-development-fixes.yml").unlink()
Path("scripts/ci/apply_hourly_development_review_fixes.py").unlink()
