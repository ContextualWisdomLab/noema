...
npm run security:private-reporting-audit      # read-only GitHub setting receipt collector
npm run security:private-reporting-authority  # validate exact-source/freshness receipt authority
npm run release:verify     # typecheck + test + security:scan + kpi:verify + acquisition:manifest
npm run release:verify:strict # production strict gate; requires fresh private-reporting receipt + production KPI evidence
```
...
`release:verify:strict` is not the pre-publication source gate. The protected CD job first verifies the checked-out immutable release source, then recollects the private-vulnerability-reporting setting for that exact commit through the repository-scoped `Metadata: read` GitHub App capability and supplies the same-run receipt to strict verification. A direct/manual strict run therefore must provide `GITHUB_REPOSITORY`, `NOEMA_PRIVATE_VULNERABILITY_REPORTING_RECEIPT_PATH`, and `NOEMA_PRIVATE_VULNERABILITY_REPORTING_EXPECTED_SOURCE_SHA` for one fresh repository/source-matching observation; an older retained PASS is not implicit authority. Use `npm run release:verify` for the ordinary pre-tag source gate.

The private-reporting read bearer and Cloudflare API bearer used by production tooling are not ambient runtime environment variables. CD bootstraps short-lived/secret values into owner-only temporary capability files, removes the raw bootstrap environment values before Node execution, and exposes only the non-secret capability-file paths to scripts. Private-reporting collection is limited to the repository-scoped `Metadata: read` App capability described in `AGENTS.md`; the release receipt does not grant setting mutation, deployment, or vulnerability-case authority. Cloudflare deployment scripts export only `NOEMA_CLOUDFLARE_API_TOKEN_PATH` and remove the capability after the mutation/status sequence. The scripts reuse the repository's bounded no-follow capability reader. `CLOUDFLARE_ACCOUNT_ID` and worker-name configuration are non-bearer deployment configuration.

Operational/audit tooling (all in `scripts/`, run via npm): `kpi:compute`, `kpi:collect`, `kpi:check`, `kpi:alerts`, `kpi:verify[:strict]` (KPI pipeline over `exchange-30d.ndjson` structured logs), `smoke:check`, `production:preflight`, `readiness:audit`, `acquisition:manifest` / `acquisition:audit`, `security:evidence`, `security:private-reporting-audit`, `security:private-reporting-authority`. The README and focused security/doctoring docs document the required `NOEMA_*` configuration; the scheduled `readiness-scan` / `acquisition-readiness-scan` workflows run their respective audits daily.
