...
export function copySourceTree(
  ...
  function copyDirectory(source, destination, isRoot) {
    for (const name of readdirSync(source).sort()) {
      ...
      if (isRoot && name === "node_modules") {
        throw new Error("source tree must not contain node_modules");
      }
...
export function runCli({
  ...
  try {
    ...
    const observedDigest = createHash("sha256").update(patchBytes).digest("hex");
