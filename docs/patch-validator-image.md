...
The image result is **validation evidence only**. It cannot approve a pull request, replace independent review, satisfy branch protection, authorize merge, publish a release, or authorize deployment.
...
- verifies bounded image metadata, smoke, and SBOM receipts; and
...
flowchart LR
    ...
    E --> F[Bounded verification receipts]

    F -. evidence only .-> G[Review evidence]
    G -. cannot replace .-> H[Independent GitHub approval]
    H --> I[Protected merge]

    C -. future separate gate .-> J[Main-only registry publication]
    J --> K[Signature, SBOM and provenance attestations]
    K --> L[Reviewed digest-lock activation]
    L -. evidence only .-> G

    I -. separate authority .-> M[Release acceptance]
    M -. separate authority .-> N[Protected deployment]
```
...
4. model judgement;
5. GitHub review publication;
6. independent approval;
7. protected merge;
8. image publication and provenance;
9. release acceptance; and
...
The runtime wrapper injects Vitest's read-only runner configuration loader so configuration can be evaluated without writing generated bundles into the authenticated input mount.
...
- any request, image digest, or result identity mismatch.
...
- one pre-created writable result file; and
- no Docker socket.
...
4. builds, scans, smokes, and verifies receipts; and
...
| `smoke-result.json` | exact request/profile/image-bound validation result |
...
The workflow runs automatically when the Dockerfile, runtime, lockfile, host image contract, verification scripts, relevant tests, or the workflow itself changes. A manual dispatch verifies the selected trusted ref but has no pull-request number to compare.
...
- unexpected network access or writable mounts;
- discovery of a shell, package manager, Git client, or credential in the runtime;
- result-file replacement or identity mismatch;
...
The reviewer must still treat image output as evidence. It must not convert a validator result into model judgement, GitHub approval, merge authority, release authority, or deployment authority.
...
The real image boundary is additionally exercised by `.github/workflows/patch-validator-image.yml` on the exact pull-request head.
