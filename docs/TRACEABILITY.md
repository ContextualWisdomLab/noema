...
| FR-018 work-conserving continuation | ADR-0002/0009 | external hourly task + PR #80 policy | remediation-policy contract | observed multi-lane scheduler execution | External task state + proposed repository implementation |
| FR-019 deliverable handoff | ADR-0002 | hourly task, PR #80 policy and PR #71 docs | documentation and remediation contracts | protected-main mixed-lane run and double exit sweep | Proposed / In review |
...
| ADR-0002 Work-conserving autonomy | FR-011, FR-018, FR-019 | scheduler prompt, PR #80, PR #71 docs | remediation-policy and documentation contracts | observed protected-main execution across blocked/actionable lanes |
...
| Vulnerability disclosure | #72/#73 | — | Policy in review; live setting/exercise external |
...
A generic scheduler error is evidence of an uncompleted invocation, not a hidden root cause and not completion. Prompt repair earns no repository completion credit. A blocked arrow defers only that lane; documentation repair is intermediate and must return to a non-conflicting source, review, stack, operational or buyer-visible lane.
...
| TRD | `docs/TRD.md` | Design sufficient; current package, runner and private-target successors must remain aligned |
...
| Security | runtime/automation threat models + #72 | Design substantial; live settings and exercises incomplete |
| Test/operability | `docs/TEST_STRATEGY.md`, `docs/OPERABILITY.md` | Design substantial; protected operational evidence incomplete |
