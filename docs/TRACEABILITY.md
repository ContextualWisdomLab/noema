...
| Cloudflare control-plane JSON negotiation | Deployment/recovery operability + issue #611 | `scripts/cloudflare-worker-deploy.mjs`, `scripts/cloudflare-worker-status.mjs`, `scripts/cloudflare-worker-recover.mjs`, shared bounded response reader | `test/cloudflare-control-plane-json-negotiation.test.ts` plus bounded-response transport suites | immutable release, governed production deployment, controlled recovery rehearsal, deployed p95/heap | Code-current by revision; protected authority depends on whether the revision is integrated |
...
| Procedural graph advisory runtime | Proposed ADR 0017 + issue #584 | protected graph/session/screening/lifecycle surfaces, `procedural-evaluation-authority.ts`, `procedural-evaluation-handoff.ts`, State / Checkpoint bounded evaluation/rejection history hardened by protected #714, `src/policy-approval/procedural-policy-approval.ts` hardened by protected #719, and `src/policy-approval/procedural-publication-preflight.ts` | hostile schema/accessor/proxy/resource-limit tests; graph/session provenance; paired holdout and exact receipt identities; evaluation-envelope binding; P-256 signed-handoff verification; canonical signature transport; expiry-at-consumption; stable signed-claim handoff identity; history CAS/replay/digest-chain/restart/capacity plus #714 pretransaction verification/complete revalidation/non-coercing digest admission; Policy / Approval approval/revocation CAS/replay/integrity plus #719 pretransaction approval-chain verification/complete transaction-local revalidation/non-coercing retained identity and digest admission; protected #603 stable double-read publication preflight, current-revocation and lookalike rejection; `publicationAuthorized:false`; `activationAuthorized:false` | live owner/Keyverse trust selection and credential custody; non-workflow current-lifecycle/revocation authority; deployed history/workflow-state/Policy-Approval/preflight compatibility and p95; immutable released cross-service contract; actual graph-publication transaction consuming the admitted preflight; canary/rollback and production outcome evidence | Implemented on protected main with bounded durable evaluation/rejection history, protected #714 State / Checkpoint hardening, protected #601/#719 Policy / Approval CAS/hardening, and protected #603 publication preflight; graph publication/activation remains explicitly unauthorized |

Cloudflare control-plane JSON negotiation is grounded in the provider's current generated-error content-negotiation contract, not in a copied provider policy: Cloudflare. (2026, May 5). *Error responses*. Cloudflare Fundamentals. https://developers.cloudflare.com/fundamentals/reference/error-responses/. Cloudflare documents HTML as the default for generated errors when `Accept` is absent or `*/*` and JSON when `application/json` is requested. Noema therefore sends `Accept: application/json` for its machine consumers but still fail-closes malformed/non-JSON/provider-error responses; this does not move Cloudflare/provider truth, destination/outbound policy, credential, quarantine/security, or release/deployment authority into Noema.
