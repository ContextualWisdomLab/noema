...
공식 OpenCode 아카이브는 고정 버전과 SHA-256으로 검증합니다. 공급자는 NVIDIA NIM 엔드포인트만 허용하고 공유, 자동 업데이트, MCP, LSP, 외부 디렉터리, 하위 에이전트, 질문 도구, 웹 검색과 웹 가져오기를 비활성화합니다. 모델 후보는 Llama Nemotron, Nemotron Super, DeepSeek 순서로 시도합니다. **후보별 900초**와 강제 종료 유예 30초를 적용합니다. 각 실패 후보의 의존성 재설치는 60초와 강제 종료 유예 10초로 별도 제한합니다. 최악의 경우 세 후보에 대해 `3 × (900 + 30 + 60 + 10) = 3,000초`를 사용하고, 최초 설정과 최종 진단에 300초를 남겨 총 3,300초가 55분 제안 job에 정확히 들어갑니다.
