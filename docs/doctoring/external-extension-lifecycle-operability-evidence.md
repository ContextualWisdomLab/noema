...
The descriptor-safe audit path computes SHA-256 from the exact same retained bytes that pass strict UTF-8, duplicate-key, and JSON parsing. It performs one descriptor read and emits only that digest with the bounded evaluator result; invalid or ambiguous evidence receives no digest. This closes the local TOCTOU gap between parsing one file image and hashing a different read, but the digest is only an artifact identity. Producer authenticity still requires the release/deployment owner to attest those exact bytes and retain an independently verified receipt bound to the immutable release, commit, workflow identity, predicate, OIDC issuer, and deployment run.

These checks make structurally incomplete evidence and evidence explicitly labeled as local, synthetic, or fixture data fail closed. They do not detect a producer that lies about those fields by themselves. `scripts/lib/external-extension-lifecycle-deployment-authority.mjs` therefore combines the retained-byte digest with canonical acquisition deployment authority and a verified lifecycle attestation receipt. Until a real deployment workflow produces that receipt and remote evidence, a PASS from either pure evaluator remains necessary but not sufficient for issue #561 completion.
...
- **Hash the evidence in a second pathname read.** Rejected because file replacement or mutation between parse and hash would give the validator a digest for bytes it did not evaluate.
...
3. Feed the audit-emitted retained-byte SHA-256 into the deployment-authority gate and have the trusted release/deployment workflow attest the exact lifecycle evidence bytes; retain the cryptographic verification receipt rather than constructing one from unverified JSON.
