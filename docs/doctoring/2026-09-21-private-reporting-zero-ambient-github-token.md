...
The exact serialization binding replaces the earlier partial YAML `uses:` recognizer. Any semantic-equivalent alternative syntax—including quoted keys or values, escapes, aliases, anchors, flow mappings or block scalars—changes the serialized job and therefore fails closed until the authority digest is deliberately reviewed and updated. The readable setup-node assertion is intentionally narrower than a YAML parser: it requires the reviewed pinned setup-node block, including its adjacent `with:` mapping and blank `token`, while the complete-job digest remains the canonical authority against alternate serialization. No product-domain, provider-routing, quarantine/security-runtime, outbound, deployment, or case-handling authority moves into Noema.
...
A readable assertion that merely searches the whole job for a pinned setup-node line and a blank `token` somewhere else was also rejected. That wording can obscure a future digest update that accidentally moves the blank token to another action. The readable assertion therefore binds the action identity and blank token within the same reviewed contiguous setup-node block; it is explanatory evidence beside, not a replacement for, the digest.
...
- Independent review of exact `1f5a5a120ea803184649e24e0ad6fc7b3419df63` found a P2 evidence-quality regression: the digest was sound, but the readable assertions separately searched for the setup-node action and `token: ""` anywhere in the job, so the prose-level security claim was not actually scoped to the action's `with:` mapping.
- RED `d5eba34c836562c32acc4040bada6cb4bbbc9a70` makes that false-PASS executable by adding a readable-contract helper that reproduces the whole-job search and requiring an unrelated-token hostile fixture to be rejected; the predecessor logic cannot satisfy the expectation.
- GREEN `d477f1b1181e17e08ef96c716e178a04bab4ad6d` scopes the readable assertion to the contiguous reviewed setup-node block containing the pinned action, Node version and blank token. RED→GREEN changes only this focused test file; the workflow and canonical job digest remain unchanged.
...
The serialization digest intentionally makes any `setting` job edit—including a benign one—fail until the digest and readable assertions are reviewed together. That maintenance cost is accepted because the job mints the bounded GitHub App capability and prior partial YAML recognizers repeatedly admitted semantic aliases. The readable contiguous-block assertion is deliberately presentation-sensitive as well, but only as review guidance; the full-job digest remains the canonical evidence boundary. A future move to a canonical YAML AST validator is acceptable only if parser provenance, duplicate-key handling, aliases, tags and GitHub Actions syntax are explicitly bounded. If tokenless public acquisition proves operationally unacceptable, rollback requires a new reviewed decision demonstrating the least additional permission needed; silently restoring `contents: read` is not an accepted fallback.
