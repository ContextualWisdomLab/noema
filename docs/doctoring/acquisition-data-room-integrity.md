...
외부 링크는 URI 형식만으로 evidence가 되지 않는다. Final-gate external material은 exact source URL, collection time, collector identity, provenance, retained local artifact path, byte count, SHA-256을 담은 bounded local verification receipt와 실제 retained artifact를 함께 검증해야 한다.
...
SLSA v1.2는 provenance를 artifact가 어디서, 언제, 어떻게 생성되었는지를 추적할 수 있는 검증 가능한 정보로 정의하고, verifier가 producer-defined expectations와 artifact/provenance를 대조해야 한다고 설명한다. Noema의 external verification receipt는 SLSA provenance 자체를 주장하지 않지만, source identity·artifact digest·collector/provenance를 명시하고 verifier가 retained bytes에 대해 expectations를 다시 적용한다는 동일한 trust principle을 따른다.
