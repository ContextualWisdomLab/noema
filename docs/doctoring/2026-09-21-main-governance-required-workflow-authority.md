...
The independent exact-head review of `aec39500c37a763bc8e488c8600948e2ffa40114` then found one P1 successor defect: four passing maintainer/reviewer readiness fixtures still generated `15_368 + index`. Only `verify` therefore retained the canonical producer while the other five required contexts failed `required_status_source_mismatch`, invalidating those PASS fixtures. Repairs `a2bb143a3d96835e5217591ea387cc42361a5ff0`, `f60a9a79ef22a9f8cde8a08179e56a32f7bf9205`, `5a34d9e268157157d325b03093db10ef4738d9c9`, and `fa7eb7ab8f17427f8b9895aa20a78913df28cf42` update `maintainer-app-governance-binding`, `maintainer-app-readiness`, `reviewer-app-identity-binding`, and production-readiness coverage fixtures to import the same canonical integration constant.

The next independent exact-head review of `5659b8d42310078189a7aa1f3be6a84d30bcf469` found a second P1 successor defect in those same four PASS fixtures: they still omitted the newly mandatory canonical `workflows` rule. Because `evaluateMaintainerAppReadiness()` passes live rules directly to `evaluateMainGovernanceRules()`, producer-correct fixtures still failed `required_security_workflow_missing`. Ordinary-forward repairs `6679afb550976682b7f63cf90641b6e5e9031b7c`, `1ad6b6bb2b11e7a565ff66c0df35d00d02800ed9`, `b7d9aaee1352cbdab38d3edb8fd78ed540b3ac31`, and `bdf19dc2de27780f68acb05f35d76f2bc0a6348f` add the same `REQUIRED_MAIN_WORKFLOW` owner/repository/path/ref authority to all four fixtures rather than inventing local workflow identities.
...
`AGENTS.md` mandatory central Security Scan/status gates → `scripts/lib/main-governance-audit.mjs` canonical required-workflow predicate + exact rule/status/producer admission → governance evaluator regressions + maintainer/reviewer readiness fixtures consuming the same producer/workflow constants → `scripts/main-governance-audit.mjs` exact protected-main collector → `artifacts/governance/main-governance-audit.json` → issue #27 live governance closure.
