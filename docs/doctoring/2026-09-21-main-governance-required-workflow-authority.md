...
The broader repository suite and hosted current-head gates remain separate evidence classes. Source reasoning is not repository full-suite, hosted current-head CI, live target-policy configuration, merge, release, or deployment evidence.
...
`AGENTS.md` mandatory central Security Scan/status gates → `scripts/lib/main-governance-audit.mjs` canonical required-workflow predicate + exact rule/status/producer admission → `test/main-governance-audit.test.ts` absence/lookalike/malformed/producer regressions + `test/main-governance-authority-exactness.test.ts` serialization and producer exactness regressions → `scripts/main-governance-audit.mjs` exact protected-main collector → `artifacts/governance/main-governance-audit.json` → issue #27 live governance closure.
