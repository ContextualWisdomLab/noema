...
그 adjacency repair 뒤 fresh independent review는 token-class false PASS를 추가로 확인했습니다. `// /** Reject invalid status authority. */`처럼 line comment 안에 JSDoc 모양의 텍스트를 넣고 바로 function declaration을 두면, predecessor helper는 문자열 위치만 보고 이를 실제 JSDoc token으로 오인했습니다. 의미 vocabulary와 종료 위치가 모두 맞더라도 주석 종류 자체가 JSDoc이 아니므로 owned production Docstring 계약의 증거가 될 수 없습니다.
...
- `direct`는 실제 JSDoc token이 독립된 comment로 시작하고, 종료 뒤 function declaration까지 whitespace 외 다른 comment/code가 없는 구조적 조건이어야 합니다.
...
5. adjacency는 prefix의 마지막 `*/`만 확인: intervening block comment를 direct JSDoc으로 오인하므로 기각했습니다.
6. `/** ... */`의 문자열 위치만 검사: line comment나 다른 lexical context 안의 JSDoc-like text를 실제 JSDoc으로 오인할 수 있어 기각했습니다. 선택한 방식은 declaration 직전 prefix의 마지막 독립 comment가 line-start whitespace 뒤의 실제 `/** ... */` 형태이고, 그 comment 내부에서 첫 `*/`가 종료점이며, 종료 뒤 declaration까지 whitespace만 존재하도록 요구합니다.
...
Adjacency RED `ed80b497c00a22be89a9d11085b8a3ce0fdd9baa`은 `/** Reject invalid status authority. */` 뒤에 unrelated block comment를 삽입한 fixture가 거부되어야 한다고 요구합니다. Predecessor helper는 의미 vocabulary를 만족하는 앞선 JSDoc과 마지막 block-comment terminator를 하나의 direct contract처럼 결합해 이 fixture를 false PASS했습니다. GREEN `291a60ce77587e647a3bf09e90cbc5eddfc99b09`은 마지막 JSDoc 시작 뒤 첫 `*/`의 위치가 trimmed declaration prefix의 끝과 정확히 같아야 한다고 요구해 JSDoc과 declaration 사이에 whitespace만 허용했습니다.

Token RED `ddcc8ad03fa7d5aa46cea84768a58d672839de2a`은 `// /** Reject invalid status authority. */` 뒤의 `weakContract()`가 거부되어야 한다는 hostile fixture를 추가했습니다. Predecessor helper는 line comment 내부의 `/**` 위치와 `*/` 종료만 보고 false PASS합니다. Token GREEN `fc4f1b6ce03034cd3c87e7eed207bd9803b8ec8a`은 declaration 직전 prefix에 대해 line-start whitespace 뒤의 독립 `/** ... */` comment 하나만 직접 JSDoc으로 인식하고, JSDoc 내부 첫 `*/` 뒤 declaration까지 whitespace만 허용하는 구조적 matcher로 교체했습니다. 이 matcher는 기존 unrelated block-comment fixture와 새 line-comment lookalike fixture를 모두 fail closed하면서 기존 의미 action/boundary 검증을 그대로 적용합니다.

38-function allowlist와 production source는 변경하지 않았습니다.

## Evidence and traceability

- Independent semantic review finding: PR #730, CodeRabbit review run `ce099b85-b0c4-4037-b8eb-e2e2465efc31`, review comment `4091398548`.
- Independent token-boundary review finding: PR #730, CodeRabbit review run `441097fd-0922-4dbc-a535-9f481a0e6266`, review comment `4092532060`.
- Pre-fix semantic exact: `1cfc08e0be9b922ba332825f1327a728afd28a8c`.
...
- Token-class RED: `ddcc8ad03fa7d5aa46cea84768a58d672839de2a`.
- Token-class GREEN repair: `fc4f1b6ce03034cd3c87e7eed207bd9803b8ec8a`.
...
Action/boundary vocabulary는 자연어 의미를 완전히 증명하는 정적 분석이 아닙니다. 직접 JSDoc 판정도 JavaScript/TypeScript 전체 lexer를 재구현하지 않고, 이 저장소의 top-level function declaration 계약에 맞춘 보수적 구조 matcher입니다. 현재 matcher는 single-token/generic `current status`형 의미 false PASS, intervening block comment, line-comment JSDoc lookalike를 차단하면서 기존 38개 production JSDoc의 서로 다른 계약 표현을 보존합니다. Independent review와 hosted exact-head tests는 계속 별도 evidence이며, current exact는 두 evidence가 완료되기 전 merge authority가 아닙니다.