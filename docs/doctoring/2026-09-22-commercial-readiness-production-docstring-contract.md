...
Current executable contract는 다음 **36개** authority-bearing production 함수를 직접 보호합니다.

- `scripts/hourly-commercial-readiness.mjs`: 19개 — retry timestamp/chronology/suite identity, required-workflow id/metadata/source, target PR association, suite authority projection, producer projection, required-workflow observation/binding, exact general review-state aggregation, review-head decision, exact Commit Status projection, snapshot assembly, SHA-bound normal merge, main loop.
...
#730의 authority-bearing production docstring contract는 현재 **36-function scope**를 executable하게 보호합니다. Fresh independent current-head review와 hosted exact-head GREEN은 별도 merge authority로 계속 필요하며, source mutation이 생기면 predecessor review/check evidence를 재사용하지 않습니다.
