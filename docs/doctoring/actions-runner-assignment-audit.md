...
The operator supplies an explicit bounded set of GitHub Actions workflow-run IDs plus the exact expected pull-request source-head SHA. The collector retrieves each selected workflow run, requires its exact positive `run_attempt`, then retrieves all job pages only for that attempt through `attempts/{run_attempt}/jobs?per_page=100`. The run-wide `filter=all` endpoint can include predecessor attempts and is therefore not current-attempt runner-assignment authority.

Required invariants are:

1. the repository is exactly `ContextualWisdomLab/noema`;
2. the production CLI reads its short-lived GitHub transport authority from an owner-only delegated token capability file named by `NOEMA_MAINTAINER_TOKEN_PATH`; the configured path is byte-canonical and is not silently trimmed or otherwise normalized, while the bearer value is never read from ambient `GH_TOKEN` and is never retained in the report;
...
5. selected runs must be `pull_request` runs bound to that exact source head and must carry an exact positive `run_attempt`;
6. job pages are fully paginated with `per_page=100` from `actions/runs/{run_id}/attempts/{run_attempt}/jobs`, bounded to at most 2,000 retained jobs; the run-wide `filter=all` endpoint must not be substituted because it can mix predecessor-attempt jobs into current-attempt evidence;
...
The production command consumes an owner-only delegated token capability file. The path itself is non-secret and must be supplied exactly: surrounding whitespace or other implicit trimming is not accepted as alternate capability authority. The token bytes must be materialized by an authorized caller and removed promptly after the audit.
...
An ambient `GH_TOKEN` is intentionally not a production credential source for this command. The descriptor-safe shared capability reader rejects non-canonical surrounding-whitespace paths, symlinks, non-regular files, group/world-readable files, wrong-owner files where UID inspection is available, oversized content, invalid UTF-8, control-bearing tokens, and file identity/version changes during the bounded read.
...
The report records repository, expected head, selected run IDs, exact run attempts, observation time, queue grace, deterministic checks/failures, and explicit false authority flags for required-check success, review, merge, release, and deployment. Temporary report bytes are created owner-only and atomically renamed onto the fixed report path.

`PASS` exits zero. `PENDING` and `FAIL` both exit nonzero. A malformed source identity, cross-repository request, missing/unsafe capability file, malformed API JSON, GitHub CLI failure, pagination-shape failure, excessive evidence, invalid/missing `run_attempt`, or head mismatch fails closed.
...
- exact positive `run_attempt` is required by both collection and evaluation, and job reads use only `attempts/{run_attempt}/jobs?per_page=100` rather than treating run-wide predecessor attempts as current authority;
...
- the production entrypoint succeeds with an exact byte-canonical owner-only delegated token capability path/file and fails closed when only ambient `GH_TOKEN` is present;
...
- the operator adapter performs only the two documented read families and fully paginates exact-attempt jobs;
