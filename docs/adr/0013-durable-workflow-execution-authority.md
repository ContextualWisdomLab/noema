...
sequenceDiagram
    ...
    O->>R: readState(plan) / validate retained authority
    R-->>O: current retained state validated or conflict
    O->>O: read SQLite databaseSize
...
- The observation-only storage-size command lets a deployed evidence producer bind later storage-growth measurements to the exact execution-scoped object without returning retained workflow state. Source/fake values alone still prove neither deployment nor a growth denominator.
