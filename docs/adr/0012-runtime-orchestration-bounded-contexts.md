...
Protected `main` is currently an evidence-producing credential and maintenance control plane. Noema is expanding toward runtime Agent/application orchestration, but that expansion must not collapse CWL domain ownership into one service or turn Noema into a model-provider router.
...
The first candidate runtime code in PR #528 introduces an Agent Runtime lifecycle and a State / Checkpoint admission primitive. These primitives need an explicit architectural decision so future workflow, tool, persistence, and integration work cannot infer broader authority from their existence.
...
On PR #528, only the following runtime behavior is implemented:
...
No checkpoint payload persistence, queue, workflow scheduler, arbitrary tool executor, provider routing, Context Assertion publisher, EA writer, or security-runtime implementation is implied by these modules. Until this ADR and code integrate into protected `main`, they remain candidate truth.
