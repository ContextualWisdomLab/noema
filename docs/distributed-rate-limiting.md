...
2. The client bucket is derived exclusively from Cloudflare's `CF-Connecting-IP` header. Caller-controlled forwarding headers are ignored. A missing, malformed, or overlong value is treated as an ingress-integrity failure and returns `503` before any shared bucket or Durable Object lookup is used.
...
7. Exercise an alarm after its original window has expired and a newer window has already begun; confirm the newer count remains stored and the alarm is moved to the newer reset deadline.
8. Temporarily test a missing or invalid binding in a non-production environment and confirm `/exchange` fails closed with `503` rather than bypassing the limiter.
9. Retain Cloudflare deployment evidence and post-deployment smoke evidence through the existing production workflow.
...
Do not restore a shared `unknown` identity bucket. If `CF-Connecting-IP` is missing or invalid, fail closed and repair the Cloudflare route or transform configuration. A shared fallback bucket allows one malformed or identity-stripped traffic source to consume the request budget for unrelated legitimate callers.
