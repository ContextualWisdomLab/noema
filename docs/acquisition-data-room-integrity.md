...
5. 작업 트리 변환을 실행할 수 있는 `git diff-files`는 호출하지 않는다. Exact HEAD의 `git ls-tree` inventory와 `O_NOFOLLOW` descriptor로 읽은 원시 바이트를 사용하고, Node `createHash`로 Git blob ID를 직접 계산해 비교한다. `--no-ext-diff`와 `--no-textconv`만으로 `clean/process` helper 실행이 차단된다고 가정하지 않는다. Mode, 크기, path/descriptor identity와 전후 metadata 검사도 유지한다. 상세 계약은 [Acquisition Git Content Authentication](acquisition-git-content-authentication.md), 결정과 회귀 근거는 [conversion-boundary ADR](adr/proposed_acquisition_git_conversion_boundary.md)에 있다.
