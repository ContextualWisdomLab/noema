...
Observed predecessor PR #556 exact `fecb03d9c632f90f290f921c1d6e90ce86ca5305`는 당시 live feature base를 포함하지 않아 hosted CI `34089768682`의 live-base guard에서 실패했다. Successor 작업은 remote writer와 base를 다시 읽고 `06ed62fcc5611e9b25ef38b06e87e7521dbf1be1`을 ordinary two-parent merge로 보존했다. `live #556 must be re-fetched before integration`은 successor publish 후에도 merge·release 직전 fresh state를 확인하는 규칙으로 유지한다. 이 문서 자체의 commit SHA를 자기 내용에 순환 참조하지 않으며, publish된 exact head와 exact-head Checks는 PR #556 body와 linked run evidence가 권위다. Fresh Security를 포함한 successor evidence 전에는 여전히 non-authorizing이다.

#556이 소유하는 source contract는 producer-issued evidence receipt, exact repository/head/workflow/run/attempt identity, claim/evidence digest, evidence-kind separation, model-visible `[receipt:<id>]` reference와 pre-publication admission이다. Attested manifest의 별도 `ClaimEvidenceRequirement`가 exact claim, independently required evidence kind, `context|finding` publication authority를 소유한다. Raw current-head source line은 `context`이며 model finding reference에서 제외되고 blocking verdict를 발행할 수 없다. Trusted source producer가 exact source defect에 `finding` authority를 명시한 경우만 source-only finding이 path/line binding을 유지해 게시될 수 있다. Remaining boundary는 central `sandboxed_verify` exact stdout/stderr/marker의 실제 manifest population, trusted research producer, fresh Security 포함 exact-head gates, immutable Noema release, released central `.github#1641` consumer bump와 original corpus RED→GREEN이다.
...
| Noema owner | stacked `noema#556` builds canonical artifacts that bind all receipt semantics and a separate authenticated claim requirement. The production source collector emits context-only entries; admission rejects those entries before deterministic gates/publication if a model attempts to promote source existence into a defect. Producer-authorized source findings remain available with exact path/line binding. | wire existing execution/research producers into the same manifest, exact-head CI/security/SBOM/provenance, normal parent integration, immutable release | Proposed; claim-level publication authority repaired locally, hosted exact-head evidence pending |
| Existing producer adapters | current-head source uses the canonical Noema producer and live checkout. The reviewed `sandboxed_verify` adapter source binds its marker, stdout/stderr and immutable helper revision, but production central workflow does not yet populate a finding-authorized manifest entry from that adapter. Research must still create a content-addressed retrieval artifact. Path trust reuses the authenticated OpenCode manifest reader precedent. | make execution/research publish canonical entries into the same attested manifest; do not translate marker text or model dictionaries into authority | Source context integrated; execution adapter source present but workflow wiring absent; research not integrated |
