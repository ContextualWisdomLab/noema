...
This revision adds a `trusted-research-retrieval@v1` anti-corruption adapter in front of that generic kernel. The adapter accepts only already-retrieved immutable bytes and a non-empty exact excerpt byte sequence, and delegates to the generic kernel only when the excerpt occurs verbatim in the retrieved bytes. It does not decode or normalize text, fold whitespace, match paraphrases, select sources, perform network retrieval, claim Zotero/OA truth, or make model-selected sources authoritative. Even after this source is integrated, the live trusted retrieval producer and retrieval-to-manifest wiring, immutable Noema release, and released central consumer remain separate #555 completion evidence.
...
| --- | --- | --- | --- | --- | --- | --- |
| P0 | Exact-claim evidence supply chain | producer evidence 없는 tool/research claim이 reviewer authority가 될 위험 | protected #556 + trusted-research adapter source + issue #555 | generic source integrated; trusted adapter source implemented; live producer/wiring/release/consumer open | canonical trusted retrieval producer + exact adapter integration + immutable Noema release + released central consumer | canonical producer wiring을 별도 TDD로 진행하고 mutable consumer bump 금지 |
