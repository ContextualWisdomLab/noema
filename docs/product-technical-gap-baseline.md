...
Current protected source는 mutation·merge·release 시점에 live protected `main`을 다시 조회해 결정한다. 이 문서 안의 exact source SHA는 dated observation 또는 protected history일 뿐 future merge 뒤 evergreen current authority로 사용하지 않는다. Dated protected observation for this repair는 `main@b3e940e4c0fb317ce50d34458bf8f2958f428cd7`이며, 이 revision은 #605의 Workflow / Task exact-object operability source, #606의 documentation-authority convergence, 그리고 #607 trusted-research byte-integrity adapter의 normal integration까지 포함한다.
...
| --- | --- | --- | --- | --- | --- | --- |
| P0 | Exact-claim evidence supply chain | producer evidence 없는 tool/research claim이 reviewer authority가 될 위험 | protected #556 + #607 + issue #555 | generic source + protected #607 adapter integrated; live producer/handoff/release/consumer open | `.github#2087` immutable trusted retrieval producer + exact Noema adapter wiring + immutable Noema release + released central consumer | #2087 immutable producer contract 후 narrow adapter wiring TDD; mutable consumer bump 금지 |
