...
Current protected source는 mutation·merge·release 시점에 live protected `main`을 다시 조회해 결정한다. 이 문서 안의 exact source SHA는 dated observation 또는 protected history일 뿐 future merge 뒤 evergreen current authority로 사용하지 않는다. Dated protected observation for this repair는 `main@48e91be43b185f37df034a787c25279221164d47`이며, GitHub-verified #614 normal merge까지 포함한다.
...
This is protected source only. ADR 0018 remains `Proposed` until an immutable reviewed release is deployed and a controlled production recovery rehearsal retains independently verified receipt/Sigstore authority, provider pre/post distribution, post-recovery smoke/KPI and immutable recovery evidence.
...
| --- | --- | --- | --- | --- | --- | --- |
| P0 | Production deployment recovery authority | split traffic/foreign Worker/stale deployment receipt가 잘못된 복구 mutation 권한으로 사용될 위험 | protected #610 + #612 + #614 / issue #611 | source integrated; controlled rehearsal/release evidence open | immutable reviewed release + controlled recovery rehearsal + provider pre/post distribution + smoke/KPI + immutable recovery receipt | immutable reviewed release 이후 actual owner-controlled recovery rehearsal과 immutable evidence 검증 |
