...
Current protected source는 mutation·merge·release 시점에 live protected `main`을 다시 조회해 결정한다. 이 문서 안의 exact source SHA는 dated observation 또는 protected history일 뿐 future merge 뒤 evergreen current authority로 사용하지 않는다. Dated protected observation for this repair는 `main@c8aeacb5919fc639cd230b199b6ee82ce99a3c3f`이며, GitHub-verified #610 normal merge까지 포함한다. #612는 이 protected source 위의 active Draft candidate이며 merge 전에는 protected authority가 아니다.
...
## Active Worker deployment recovery candidate — issue #611 / Draft PR #612

Protected #610 moved production Cloudflare mutation/status/evidence off Wrangler runtime/status calls onto repository-owned direct API clients and binds deployment evidence to exact clean source, Worker version/deployment UUID and post-deployment status. #612 is the active single-writer repair for the remaining recovery-authority gap: pre-mutation split traffic must not collapse to provider array position or one selected version.

Candidate #612 preserves the complete pre-mutation deployment UUID, observation/creation time and canonical one/two-version percentage distribution; it makes `restore_exact_pre_deployment_distribution` explicit, keeps legacy `previousWorkerVersionId` only for an unambiguous single-version 100% state, and validates a recovery mutation against the receipt Worker identity plus a fresh current active deployment read immediately before mutation. Cross-Worker receipt substitution, stale current deployment, malformed UUID/percentage/distribution, duplicate version, missing observation and first-deployment/no-prior-state ambiguity fail closed.

This is candidate source only. Normal merge still requires one unchanged exact head with application CI, reviewer-ci, required Security Scan and patch-validator-image terminal GREEN plus clean current-head review/thread state. Even after source integration, ADR 0018 remains `Proposed` until an immutable reviewed release is deployed and a controlled production recovery rehearsal retains independently verified receipt/Sigstore authority, provider pre/post distribution, post-recovery smoke/KPI and immutable recovery evidence.
...
| --- | --- | --- | --- | --- | --- | --- |
| P0 | Production deployment recovery authority | split traffic/foreign Worker/stale deployment receipt가 잘못된 복구 mutation 권한으로 사용될 위험 | protected #610 + issue #611 / active Draft #612 | candidate source under review; rehearsal/release evidence open | exact-head GREEN merge + immutable reviewed release + controlled recovery rehearsal + provider pre/post distribution + smoke/KPI + immutable recovery receipt | #612 current exact hosted gates/review 수렴 후 normal merge; source integration 뒤 실제 owner-controlled rehearsal |
