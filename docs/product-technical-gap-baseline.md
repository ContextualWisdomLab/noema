...
Open PR #607은 이 generic kernel 위에 `trusted-research-retrieval@v1` 전용 anti-corruption adapter를 추가하는 active candidate다. Adapter는 이미 retrieval된 immutable bytes와 non-empty exact excerpt bytes만 받고, excerpt가 retrieved bytes의 exact slice로 실제 존재할 때만 generic kernel에 위임한다. Unicode decoding/normalization, whitespace folding, paraphrase matching, source selection, network retrieval, Zotero/OA truth 또는 model-selected source authority는 만들지 않는다. 이 candidate가 보호된 source로 병합되더라도 live trusted retrieval producer와 retrieval-to-manifest wiring, immutable Noema release, released central consumer는 #555의 별도 completion evidence로 남는다.
...
| --- | --- | --- | --- | --- | --- | --- |
| P0 | Exact-claim evidence supply chain | producer evidence 없는 tool/research claim이 reviewer authority가 될 위험 | protected #556 + candidate #607 + issue #555 | generic source integrated; trusted adapter candidate; live producer/wiring/release/consumer open | canonical trusted retrieval producer + exact adapter integration + immutable Noema release + released central consumer | candidate exact-head gates/merge 뒤 canonical producer wiring을 별도 TDD로 진행하고 mutable consumer bump 금지 |
