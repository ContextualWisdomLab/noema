...
| Noema owner | stacked `noema#556` builds canonical artifacts that bind all receipt semantics; only an exact-digest/canonical OpenCode-handoff envelope satisfying reviewed producer-kind and repo/head/workflow/run/attempt policy can create the immutable receipt-ID index exposed to admission | exact-head CI/security/SBOM/provenance, actual producer and agent→gate→publish wiring, normal parent integration, immutable release | Proposed |
