...
- Cloudflare deploy/status/recovery control-plane JSON은 응답 전체를 먼저 buffering하지 않고 공통 bounded streaming reader로 읽는다. 1 MiB ceiling은 chunk 수신 중에 적용하며 초과 chunk를 수용하기 전에 거부하고, stream cancellation 실패는 원래 oversized-response failure를 덮지 않아야 한다. Reader lock은 성공·실패 모두 반환하며 invalid UTF-8, non-JSON, HTTP/provider-declared error는 실패-폐쇄한다. 이 transport hardening은 recovery authority나 production rehearsal evidence를 새로 만들지 않는다;
- treat the create-deployment response as mutation acknowledgement only. After mutation, perform a fresh post-mutation provider status read, require the active recovery deployment UUID and exact canonical version/percentage distribution to match the requested recovery state, then re-run `/health`, `/ready`, `/exchange` smoke and retain a separate immutable recovery record. A successful source-level command output is mutation evidence, not proof that a controlled production recovery rehearsal completed;
...
Protected #612/#614/#616 recovery source preserves the complete pre-mutation Cloudflare traffic distribution, separates exact-state restoration from deliberate single-version rollback, requires current Worker/deployment identity before mutation, canonicalizes valid UUID textual aliases to one lowercase identity before recovery-authority comparison/deduplication/ordering/retention/mutation planning, and treats the create-deployment response as mutation acknowledgement rather than active-state authority. Protected #616 requires a fresh post-mutation provider status read to show the newly created recovery deployment and exact canonical distribution before command success. It is protected recovery source, but it is not operational proof: immutable release, governed deployment, and a controlled production recovery rehearsal with retained pre/post provider state, smoke/KPI and immutable recovery evidence remain required. ADR-0018 remains `Proposed`.

PR #618 adds a source-level transport invariant to those Cloudflare control-plane paths: deploy/status/recovery response bodies are consumed incrementally under a 1 MiB ceiling instead of calling `response.text()` before size validation. Oversized streams fail before accepting the over-limit chunk; cleanup/cancellation cannot convert that failure into success, and invalid UTF-8/JSON/provider errors remain fail-closed. This hardening does not establish an immutable release, governed deployment, recovery rehearsal, p95/SLO, or production outcome by itself.
